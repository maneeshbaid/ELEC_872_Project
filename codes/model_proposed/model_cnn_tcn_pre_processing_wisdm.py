...
# Removed keras import as TensorFlow 2's Keras is used
def model(x_train, num_labels, tcn_filters, num_conv_filters, batch_size, num_heads, key_dim):
    """
    The proposed model with CNN layer, TCN layers.
    Inputs:
    ...
    - model: A Keras model
    """
    cnn_inputs = Input(shape=(x_train.shape[1], x_train.shape[2]), batch_size=batch_size, name='cnn_inputs')
    cnn_layer = Conv1D(filters=num_conv_filters, kernel_size=3, padding='same', activation='relu', name='cnn_conv1d')(
        cnn_inputs)

    sq_layer_out = cnn_layer  # No need for redundant output, using cnn_layer directly

    tcn_layer_1 = Conv1D(filters=tcn_filters, kernel_size=3, padding='causal', activation=None,
                         kernel_regularizer=l2(1e-5), name='tcn_conv1')(sq_layer_out)
    tcn_layer_1 = BatchNormalization()(tcn_layer_1)
    tcn_layer_1 = ReLU()(tcn_layer_1)
    tcn_layer_1 = Dropout(0.1)(tcn_layer_1)

    tcn_layer_2 = Conv1D(filters=tcn_filters, kernel_size=3, padding='causal', activation=None,
                         kernel_regularizer=l2(1e-5), name='tcn_conv2')(tcn_layer_1)
    ...
    combined_output = tcn_layer_output

    pooled_output = tf.keras.layers.GlobalAveragePooling1D()(combined_output)
...
SAVE_DIR = './model_with_wisdm_pre_processing_cnn_tcn_' + MODE + '_results'
...
if __name__ == '__main__':
    ...
    random.seed(SEED)
    np.random.seed(SEED)
    tf.random.set_seed(0)  # Updated for TensorFlow 2
    ...
    for DATA_FILE in DATA_FILES:
        ...
        # Standardization (fit on entire dataset)
        scaler = StandardScaler()
        X_flat = X.reshape(-1, X.shape[-1])  # Flatten the features for scaling
        X_scaled = scaler.fit_transform(X_flat)
        X = X_scaled.reshape(X.shape)

        # Data Augmentation - Time Warping and Adding Noise
        augmenter = (TimeWarp() * 3) + AddNoise(scale=0.01)
        X_aug = augmenter.augment(X)

        # Combine original and augmented data
        X = np.concatenate((X, X_aug), axis=0)
        y_one_hot = np.concatenate((y_one_hot, y_one_hot), axis=0)
